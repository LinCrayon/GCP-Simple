...
func live(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Log Println error if the WebSocket upgrade fails (e.g., invalid request headers).
		log.Println("model closed:", err)
		return
	}
	defer c.Close()
	...
	if err != nil {
		// Log Println error if client creation fails (e.g., invalid config, authentication issues).
	...
	if err != nil {
		// Log Println error if connecting to the model fails (e.g., network issues, invalid model name).
		log.Println("connect to model error: ", err)
		return
	}
	...
	go func() {
		for {
			...
			if err != nil {
				// Log Println error if receiving from the GenAI service fails (e.g., connection closed, network error).
				log.Println("receive model response error: ", err)
			}
			// Marshal the received message into JSON format.
			messageBytes, err := json.Marshal(message)
			if err != nil {
				// Log Println error if marshaling the message to JSON fails.
				log.Println("marhal model response error: ", message, err)
			}
	...
	for {
		...
		if err := json.Unmarshal(message, &realtimeInput); err != nil {
			// Log Println error if unmarshaling the client message fails (e.g., invalid JSON format).
...
func main() {
	...
	// Log Println error if the HTTP server fails to start (e.g., port already in use).
